...
create table if not exists public.intent_communities (
  intent_id uuid not null references public.intents(id) on delete cascade,
  community_id uuid not null references public.communities(id) on delete restrict,
  position smallint not null,
  created_at timestamptz not null default now(),
  primary key (intent_id, community_id),
  constraint intent_communities_position_check check (position between 0 and 2),
  constraint intent_communities_position_unique unique (intent_id, position)
);

create index if not exists intent_communities_community_intent_idx
on public.intent_communities (community_id, intent_id);

alter table public.intent_communities enable row level security;
revoke all on public.intent_communities from anon, authenticated;

insert into public.intent_communities (intent_id, community_id, position)
select intent.id, intent.community_id, 0
from public.intents intent
where intent.community_id is not null
on conflict (intent_id, community_id) do update set position = excluded.position;
...
create or replace function public.replace_intent_communities_internal(
  p_intent_id uuid,
  p_activity_id uuid,
  p_community_ids uuid[]
)
...
declare
  v_ids uuid[];
begin
  v_ids := public.validate_intent_community_ids(p_activity_id, p_community_ids);

  delete from public.intent_communities where intent_id = p_intent_id;

  insert into public.intent_communities (intent_id, community_id, position)
  select p_intent_id, value, (ordinal - 1)::smallint
  from unnest(v_ids) with ordinality as item(value, ordinal);

  update public.intents
  set community_id = v_ids[1], updated_at = now()
  where id = p_intent_id;
end;
$$;

create or replace function public.sync_legacy_intent_primary_community()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  if new.community_id is distinct from old.community_id then
    if new.community_id is null then
      delete from public.intent_communities where intent_id = new.id;
    elsif not exists (
      select 1 from public.intent_communities
      where intent_id = new.id and position = 0 and community_id = new.community_id
    ) then
      delete from public.intent_communities where intent_id = new.id;
      insert into public.intent_communities(intent_id, community_id, position)
      values (new.id, new.community_id, 0)
...
drop trigger if exists sync_legacy_intent_primary_community_trigger on public.intents;
create trigger sync_legacy_intent_primary_community_trigger
after update of community_id on public.intents
for each row execute function public.sync_legacy_intent_primary_community();

create or replace function public.sync_inserted_intent_primary_community()
returns trigger
language plpgsql
security definer
set search_path = public
...
drop trigger if exists sync_inserted_intent_primary_community_trigger on public.intents;
create trigger sync_inserted_intent_primary_community_trigger
after insert on public.intents
for each row execute function public.sync_inserted_intent_primary_community();

create or replace function public.create_my_intent_with_communities(
  p_start_date date,
  p_end_date date,
  p_people text,
  p_location_id uuid,
...
begin
  ...
  if p_start_date is null or p_end_date is null or p_end_date < p_start_date then raise exception 'Enter a valid Intent date range.' using errcode='22023'; end if;
  if p_budget is not null and p_budget < 0 then raise exception 'Budget cannot be negative.' using errcode='22023'; end if;
  if p_max_participants is not null and p_max_participants < 1 then raise exception 'Participant capacity must be at least 1.' using errcode='22023'; end if;
  if not exists (select 1 from public.activities a join public.activity_categories c on c.id=a.category_id where a.id=p_activity_id and a.is_active and c.is_active) then raise exception 'The selected Activity is not available.' using errcode='22023'; end if;
  if not exists (select 1 from public.locations l where l.id=p_location_id) then raise exception 'The selected location is not available.' using errcode='22023'; end if;
  if p_professional_requirement not in ('none','preferred','required') then raise exception 'Unsupported professional requirement.' using errcode='22023'; end if;
  if p_professional_requirement <> 'none' and p_professional_role_id is null then raise exception 'Select the professional role.' using errcode='22023'; end if;

  v_ids := public.validate_intent_community_ids(p_activity_id, p_community_ids);

  insert into public.intents (
    user_id,start_date,end_date,people,location_id,activity_id,budget,recurrence,visibility,notes,intent_type,max_participants,
    community_id,status,recruitment_status,matching_status,professional_requirement,professional_role_id,professional_preference_updated_at
  ) values (
    v_user_id,p_start_date,p_end_date,p_people,p_location_id,p_activity_id,p_budget,p_recurrence,p_visibility,nullif(btrim(coalesce(p_notes,'')),''),
    p_intent_type,p_max_participants,v_ids[1],'active','open','open',p_professional_requirement,
    case when p_professional_requirement='none' then null else p_professional_role_id end,
    case when p_professional_requirement='none' then null else now() end
  ) returning id into v_intent_id;

  perform public.replace_intent_communities_internal(v_intent_id,p_activity_id,v_ids);
  return v_intent_id;
end;
$$;

create or replace function public.update_my_intent_with_context_links_and_communities(
  p_intent_id uuid,
  p_activity_id uuid,
  p_location_id uuid,
  p_start_date date,
...
begin
  ...
  v_result := public.update_my_intent_with_context_and_links(
    p_intent_id,p_activity_id,p_location_id,p_start_date,p_end_date,p_people,p_recurrence,p_visibility,p_budget,p_max_participants,p_notes,v_ids[1],p_links
  );

  perform public.replace_intent_communities_internal(p_intent_id,p_activity_id,v_ids);
  return v_result;
end;
$$;

create or replace function public.get_my_intent_communities(p_intent_id uuid)
returns table(community_id uuid, position smallint)
language sql
stable
security definer
set search_path = public
as $$
  select link.community_id, link.position
  from public.intent_communities link
  join public.intents intent on intent.id=link.intent_id
  where link.intent_id=p_intent_id and intent.user_id=auth.uid()
  order by link.position;
$$;

drop function if exists public.get_visible_intent_communities(uuid[]);
create function public.get_visible_intent_communities(p_intent_ids uuid[])
returns table (
  intent_id uuid,
  community_id uuid,
  community_name text,
...
begin
  if v_user_id is null then raise exception 'Authentication is required.' using errcode='42501'; end if;
  return query
  select intent.id, community.id, community.name, community.slug, community.description,
    community.icon_key, community.icon_url, community.accent_color, community.secondary_color,
    community.scope_type, community.category_id, community.status, link.position, link.position=0
  from public.intents intent
  join public.intent_communities link on link.intent_id=intent.id
  join public.communities community on community.id=link.community_id
  where intent.id=any(coalesce(p_intent_ids,array[]::uuid[]))
    and community.status='active'
    and public.can_user_view_intent_activity(intent.id,v_user_id)
  order by intent.id, link.position, community.name;
end;
$$;
...
begin
  ...
  with plan_resources as (
    ...
    where
      exists (
        select 1
        from public.intent_communities intent_community
        where intent_community.intent_id = host_intent.id
          and intent_community.community_id = p_community_id
      )
  ...
  unlinked_intent_resources as (
    ...
    where
      exists (
        select 1
        from public.intent_communities intent_community
        where intent_community.intent_id = intent.id
          and intent_community.community_id = p_community_id
      )
  ...
  with plan_resources as (
    ...
    where
      exists (
        select 1
        from public.intent_communities intent_community
        join public.community_follows followed_community
          on followed_community.community_id = intent_community.community_id
        where intent_community.intent_id = host_intent.id
          and followed_community.user_id = v_user_id
      )
...
revoke all on function public.validate_intent_community_ids(uuid,uuid[]) from public;
revoke all on function public.replace_intent_communities_internal(uuid,uuid,uuid[]) from public;
revoke all on function public.create_my_intent_with_communities(date,date,text,uuid,uuid,numeric,text,text,text,text,integer,uuid[],text,uuid) from public;
grant execute on function public.create_my_intent_with_communities(date,date,text,uuid,uuid,numeric,text,text,text,text,integer,uuid[],text,uuid) to authenticated;
revoke all on function public.update_my_intent_with_context_links_and_communities(uuid,uuid,uuid,date,date,text,text,text,numeric,integer,text,uuid[],jsonb) from public;
grant execute on function public.update_my_intent_with_context_links_and_communities(uuid,uuid,uuid,date,date,text,text,text,numeric,integer,text,uuid[],jsonb) to authenticated;
revoke all on function public.get_my_intent_communities(uuid) from public;
grant execute on function public.get_my_intent_communities(uuid) to authenticated;
revoke all on function public.get_visible_intent_communities(uuid[]) from public;
grant execute on function public.get_visible_intent_communities(uuid[]) to authenticated;
