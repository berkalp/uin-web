...
-- ============================================================
-- 1. DISCOVERY SUPPORTING INDEXES
-- ============================================================

create index if not exists
  intents_discovery_state_idx
on public.intents (
  status,
  recruitment_status,
  matching_status,
...
create index if not exists
  intents_discovery_activity_idx
on public.intents (
  activity_id,
  end_date,
  created_at desc
...
create index if not exists
  intents_discovery_location_idx
on public.intents (
  location_id,
  end_date,
  created_at desc
...
on public.activities (
  public.normalize_activity_catalogue_name(name)
);

create index if not exists
  activity_categories_normalized_name_idx
...
on public.locations (
  public.normalize_activity_catalogue_name(district)
);

create index if not exists
  locations_normalized_city_idx
on public.locations (
  public.normalize_activity_catalogue_name(city)
);

-- ============================================================
-- 2. DISCOVERY FILTER CATALOGUE
-- ============================================================

create or replace function
  public.get_intent_discovery_filters()
returns jsonb
language plpgsql
stable
security definer
...
-- ============================================================
-- 3. VISIBILITY-SAFE INTENT DISCOVERY
-- ============================================================
