...
function getIntentShareContent({
  ...
  const title =
    `${hostName}'s ${activity.title} Intent`;

  const parts = [
    `${hostName} has a ${activity.title} Intent on UIN.`,
    targetWindow
...
export default async function ActivityDetailPage({
  ...
  const planCoverPromise = activity.plan_id
  ...
  if (planCoverResult.error) {
    console.error(
      "Plan cover lookup failed:",
      planCoverResult.error
    );
  }
  ...
  const planCoverRows =
    (planCoverResult.data ?? []) as PlanCoverRow[];

  const coverUrl = resolveActivityCover({
    planCoverUrl:
      planCoverRows[0]?.cover_url ?? null,
    activityCoverUrl:
                ...
                <div className="rounded-2xl bg-gray-50 p-4">
                  ...
                  <p className="mt-2 text-lg font-black text-gray-950">
                    {activity.budget !== null
                      ? `${Number(activity.budget).toLocaleString("en-US")} TL`
                      : "Not set"}
                  </p>
                </div>
