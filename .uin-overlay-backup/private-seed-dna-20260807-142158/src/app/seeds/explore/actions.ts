...
export async function suggestAndPlantSeedSubject(
  ...
  const itemKind = getText(formData, "item_kind") || "generic";
  ...
  if (!seedTypeId || !canonicalTitle) {
    redirect(
      appendNotice(
        returnTo,
        "error",
        "Seed Type and subject title are required."
      )
    );
  }
  ...
  const { data: catalogItemId, error: suggestionError } =
    await supabase.rpc("suggest_seed_catalog_item", {
      ...
      p_release_year:
        releaseYear !== null && Number.isFinite(releaseYear)
          ? releaseYear
          : null,
      p_cover_url: null,
  ...
  if (suggestionError || !catalogItemId) {
    redirect(
      appendNotice(
        returnTo,
        "error",
        suggestionError?.message || "Subject could not be added."
      )
    );
  }

  const { data: seedId, error: plantError } = await supabase.rpc(
    "plant_seed_from_catalog",
    {
      p_catalog_item_id: catalogItemId,
      p_visibility: "only_me",
  ...
  if (plantError) {
    redirect(appendNotice(returnTo, "error", plantError.message));
  }
