...
export default async function TimelinePage({
  ...
  function getCompactPlanPresentation(entry: PlanTimelineEntry) {
    const activity = getFirst(entry.plan.activities);
    const location = getFirst(entry.plan.locations);
    const presentation = privatePresentationByPlanId.get(entry.plan.id) ?? null;
    const title =
      presentation?.viewer_can_see_title && presentation.custom_title
        ? presentation.custom_title
        : entry.plan.title || activity?.name || "Activity";
    const coverUrl =
      presentation?.viewer_can_see_cover
        ? presentation.signed_experience_cover_url ??
          presentation.visible_cover_url ??
          activity?.default_cover_url ??
          getFirst(activity?.activity_categories)?.default_cover_url ??
          null
        : activity?.default_cover_url ??
          getFirst(activity?.activity_categories)?.default_cover_url ??
          null;
    ...
    return {
      ...
      locationLabel: [location?.district, location?.city]
        .filter(Boolean)
        .join(", "),
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-amber-900/70">
                  Only actions that can move an Intent or Activity forward appear here.
                </p>
              </div>
              <span className="rounded-full bg-white px-3 py-1.5 text-xs font-black text-amber-800 shadow-sm">
                {attentionEntries.length +
                  ...
                  pendingIntentInvitationCount +
                  pendingManagedProfileActionCount}{" "}
                action{attentionEntries.length +
                  pendingJoinRequestCount +
                  pendingIntentInvitationCount +
                  pendingManagedProfileActionCount ===
                1
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              {attentionEntries.slice(0, 4).map((entry) => {
                ...
                return (
                  ...
                  >
                    ...
                    <h3 className="mt-2 line-clamp-2 font-black text-gray-950">
                      {info.title}
                    </h3>
              ...
              {pendingJoinRequestCount > 0 && (
                ...
                >
                  ...
                  <h3 className="mt-2 font-black text-gray-950">
                    {pendingJoinRequestCount} request{pendingJoinRequestCount === 1 ? "" : "s"} waiting
                  </h3>
                  <p className="mt-2 text-xs text-gray-500">
                    Review who wants to join your Intents.
                  </p>
                </Link>
              )}
              ...
              {pendingIntentInvitationCount > 0 && (
                ...
                >
                  ...
                  <h3 className="mt-2 font-black text-gray-950">
                    {pendingIntentInvitationCount} invitation{pendingIntentInvitationCount === 1 ? "" : "s"} waiting
                  </h3>
                  <p className="mt-2 text-xs text-gray-500">
                    Accept or decline direct Intent invitations.
                  </p>
                </Link>
              )}
              ...
              {pendingManagedProfileActionCount > 0 && (
                ...
                >
                  ...
                  <h3 className="mt-2 font-black text-gray-950">
                    {pendingManagedProfileActionCount} family action{pendingManagedProfileActionCount === 1 ? "" : "s"}
                  </h3>
                  <p className="mt-2 text-xs text-gray-500">
                    Review pending managed-profile invitations.
                  </p>
                </Link>
              )}
            </div>
                    ...
                    return (
                      ...
                      >
                        <Link href={viewHref} className="block">
                          <div className="relative h-28 overflow-hidden bg-gradient-to-br from-blue-950 via-blue-700 to-cyan-500">
                            {info.coverUrl && (
                              <img
                                src={info.coverUrl}
                                alt=""
                            ...
                            <span className="absolute left-3 top-3 rounded-full bg-blue-100 px-2.5 py-1 text-[9px] font-black uppercase tracking-wide text-blue-800">
                              Planned
                            </span>
                            <h3 className="absolute inset-x-0 bottom-0 line-clamp-2 p-4 text-base font-black text-white">
                              {info.title}
                            </h3>
                          </div>
                        </Link>
