...
function friendName(context: IntentReactionContext) {
  ...
  return first.full_name || first.username || "A friend";
}
...
export default function IntentReactionBar({
  ...
  const [pendingType, setPendingType] = useState<IntentReactionType | null>(
    null
  );
  ...
  function toggle(reactionType: IntentReactionType) {
    const canToggle =
      reactionType === "save" ? canToggleSave : canTogglePaw;

    if (!canToggle || isPending) {
      setMessage(
        context.reaction_disabled_reason ||
          (isAuthenticated
            ? "This Intent is not accepting new reactions."
            : "Sign in to Save or Paw this Intent.")
      );
      return;
    }
      ...
      try {
        ...
        setMessage(
          reactionType === "save"
            ? nextActive
              ? "Saved privately."
              : "Removed from Saved Intents."
            : nextActive
              ? "Paw left. Your visibility setting applies."
              : "Paw removed."
        );
      } catch (error) {
        setMessage(
          error instanceof Error
            ? error.message
            : "The Intent reaction could not be saved."
        );
      } finally {
        setPendingType(null);
      }
    ...
    <div className={isDetail ? "rounded-2xl border border-amber-100 bg-amber-50/60 p-4" : ""}>
        ...
        {!isOwner ? (
          <>
            ...
            <button
              ...
              title={
                context.viewer_pawed
                  ? "Remove your Paw"
                  : "Recommend this Intent with a Paw"
              }
            ...
            >
              ...
              <span>{context.viewer_pawed ? "Pawed" : "Paw"}</span>
              {context.paw_count > 0 && (
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className="rounded-xl border border-rose-100 bg-rose-50 px-3 py-2 font-semibold text-rose-700">
              ♥ {context.save_count} saves
            </span>
            <span className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 font-semibold text-amber-900">
              🐾 {context.paw_count} paws
            </span>
          </div>
        )}
      ...
      {context.friend_paw_count > 0 && (
        <div className="mt-2 flex min-w-0 items-center gap-2 text-xs font-semibold text-amber-900">
          ...
          <span className="min-w-0 truncate">
            {firstFriendName}
            {additionalFriends > 0
              ? ` and ${additionalFriends} friend${additionalFriends === 1 ? "" : "s"}`
              : ""}{" "}
            Pawed this
          </span>
        </div>
      ...
      {message && (
        <p
          className={`mt-2 text-xs leading-5 ${
            message.includes("could not") || message.includes("not ")
              ? "text-red-700"
              : "text-gray-500"
          }`}
          role="status"
      ...
      {isDetail && !message && !isOwner && (
        <p className="mt-2 text-xs leading-5 text-gray-500">
          Save is private. A Paw is a social recommendation using your profile visibility setting.
        </p>
      )}
    </div>
  );
