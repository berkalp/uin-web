    ...
    try {
      ...
      if (sourceSeed && selectedSourceSeedIds.length > 0) {
        for (const seedId of selectedSourceSeedIds) {
          try {
            await linkSeedToIntent(seedId, createdIntentId);
          } catch (linkError) {
            console.warn(
              "Intent created, but one Seed lineage link could not be saved:",
              linkError
            );
          }
        }
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0 flex-1">
                    ...
                    return (
                      <label key={candidate.seed_id} className={`flex cursor-pointer items-start gap-3 rounded-2xl border p-3 transition ${selected ? "border-green-500 bg-white" : "border-green-100 bg-white/60 hover:border-green-300"}`}>
                        <input
                          type="checkbox"
                          checked={selected}
                          disabled={candidate.is_primary}
                          onChange={(event) => {
                            setSelectedSourceSeedIds((current) =>
                              event.target.checked
                                ? Array.from(new Set([...current, candidate.seed_id]))
                                : current.filter((id) => id !== candidate.seed_id)
                            );
                          }}
                        ...
                        <span className="min-w-0">
                          <span className="block truncate text-sm font-black text-gray-950">
                            {candidate.seed_type_icon} {candidate.seed_title}
                          </span>
                          <span className="mt-1 block text-[11px] font-bold uppercase tracking-wide text-gray-500">
                            {candidate.seed_scope === "private" ? "🔒 Private Seed" : "Library Seed"}{candidate.is_primary ? " · Primary" : ""}
                          </span>
                        </span>
                      </label>
                    );
                ...
                {sourceSeed.suggested_activity_name && (
                  <p className="mt-3 text-xs font-semibold text-violet-700">Suggested Activity · {sourceSeed.suggested_activity_name}</p>
                )}
              </div>
            </div>
          </section>
