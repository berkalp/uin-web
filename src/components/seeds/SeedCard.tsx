...
export default function SeedCard({
  ...
  async function remove() {
    const confirmed = window.confirm(
      isPrivateSeed
        ? `Delete private Seed “${seed.title}”?`
    ...
    <article className="flex h-full flex-col overflow-hidden rounded-[28px] border border-gray-200 bg-white shadow-sm">
      ...
      <div className="flex flex-1 flex-col p-5">
        <div className="flex flex-wrap gap-2 text-[11px] font-semibold">
          <span className={`rounded-full px-3 py-1.5 ${isPrivateSeed ? "bg-gray-950 text-white" : "bg-emerald-50 text-emerald-800"}`}>
            {isPrivateSeed ? "🔒 Private" : "Library Seed"}
          </span>
          {!isPrivateSeed && seed.catalogue_status === "pending" && (
            <span className="rounded-full bg-amber-50 px-3 py-1.5 text-amber-800">Pending Library review</span>
          )}
          {!isPrivateSeed && seed.catalogue_status === "under_review" && (
            <span className="rounded-full bg-red-50 px-3 py-1.5 text-red-700">Library subject under review</span>
          )}
          {!isPrivateSeed && (
