...
export default function SeedCard({
  ...
  return (
    <article className="flex h-full min-w-0 flex-col overflow-hidden rounded-[20px] border border-gray-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      <Link
      ...
      >
        ...
        <div className="absolute inset-x-0 top-0 flex items-start justify-between gap-2 p-2.5">
          <span className="max-w-[68%] truncate rounded-full border border-white/15 bg-black/45 px-2 py-1 text-[8px] font-black uppercase tracking-wide text-white backdrop-blur">
            {seed.seed_type_icon} {seed.seed_type_name}
          </span>
          <span className={`rounded-full px-2 py-1 text-[8px] font-black uppercase tracking-wide ${statusTone(seed.status)}`}>
        ...
        <div className="absolute inset-x-0 bottom-0 p-3 text-white">
          <div className="flex min-w-0 items-center gap-2">
            <span
              className="inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-black/45 text-white backdrop-blur"
              title={isPrivateSeed ? "Özel Tohum" : "Kütüphane Tohumu"}
              aria-label={isPrivateSeed ? "Özel Tohum" : "Kütüphane Tohumu"}
            >
              {isPrivateSeed ? <LockIcon className="h-3.5 w-3.5" /> : <LibraryIcon className="h-3.5 w-3.5" />}
            </span>
            <h2 className="min-w-0 line-clamp-2 text-base font-black leading-tight">{seed.title}</h2>
          </div>
          {seed.subtitle && <p className="mt-1 truncate pl-8 text-[10px] font-semibold text-white/75">{seed.subtitle}</p>}
        </div>
      </Link>
      ...
      <div className="flex flex-1 flex-col p-2.5">
        {!detailsOpen ? (
          <div className="grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-gray-100 bg-gray-100">
            <div className="flex h-10 items-center gap-2 bg-white px-2.5 text-[10px] font-bold text-gray-700" title="Ekildi">
              <span className="text-green-700"><SeedPlantedIcon /></span>
              <span className="truncate">{formatDate(seed.created_at)}</span>
            </div>
            <div className="flex h-10 items-center gap-2 bg-white px-2.5 text-[10px] font-bold text-gray-700" title="Son güncelleme">
              <span className="text-blue-700"><UpdatedIcon /></span>
              <span className="truncate">{formatDate(seed.updated_at)}</span>
            </div>
            <div className="flex h-10 items-center gap-2 bg-white px-2.5 text-[10px] font-bold text-gray-700" title="Hedef tarihi">
              <span className="text-rose-600"><TargetIcon /></span>
              <span className="truncate">{seed.target_date ? formatDate(seed.target_date) : "—"}</span>
            </div>
            <div className="flex h-10 items-center bg-white px-2.5" title="Kalan süre">
          ...
          <div className="min-h-[81px] rounded-xl border border-gray-100 bg-gray-50 p-3 text-[10px] text-gray-600">
            <div className="grid grid-cols-2 gap-2">
              <div><p className="text-[8px] font-black uppercase text-gray-400">Görünürlük</p><p className="mt-0.5 font-bold text-gray-900">{isPrivateSeed ? "Yalnızca sen" : getSeedVisibilityLabel(seed.visibility)}</p></div>
              <div><p className="text-[8px] font-black uppercase text-gray-400">Durum</p><p className="mt-0.5 font-bold text-gray-900">{seed.status === "active" ? "Growing" : seed.status === "completed" ? "Completed" : "Archived"}</p></div>
              <div><p className="text-[8px] font-black uppercase text-gray-400">Günlük</p><p className="mt-0.5 font-bold text-gray-900">{journalCount}</p></div>
              <div><p className="text-[8px] font-black uppercase text-gray-400">Büyüyen Niyet</p><p className="mt-0.5 font-bold text-gray-900">{grownIntentCount}</p></div>
            </div>
            {seed.notes && <p className="mt-2 line-clamp-2 leading-4">{seed.notes}</p>}
        ...
        <div className="mt-auto flex h-10 items-center gap-1.5 border-t border-gray-100 pt-2">
          <Link
            href={`/seeds/${encodeURIComponent(seed.seed_id)}`}
          ...
          <button
            ...
            className="ml-auto inline-flex h-7 items-center rounded-lg border border-gray-200 bg-white px-2.5 text-[9px] font-black text-gray-700 hover:border-green-300"
          >
