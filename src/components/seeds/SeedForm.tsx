...
export default function SeedForm({
  ...
  const isPrivateSeed = forcePrivate || seed?.seed_scope === "private";
  const isPendingLibrarySeed = catalogueIdentity?.catalogue_status === "pending" || catalogueIdentity?.catalogue_status === "under_review";
              ...
              <div>
                ...
                <p className="mt-1 text-xs leading-5 text-gray-600">
                  {isCatalogueSeed
                    ...
                    : "Private Seed · only you can see it."}
                </p>
              </div>
            </div>
          ) : (
                ...
                <div className="p-5 md:p-6">
                  ...
                  {seed?.private_origin_title && (
                    <div className="mt-4 rounded-2xl border border-indigo-100 bg-indigo-50 px-4 py-3">
                      <p className="text-[11px] font-black uppercase tracking-wide text-indigo-600">Originally captured privately</p>
                      <p className="mt-1 text-sm font-semibold text-indigo-950">{seed.private_origin_title}</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          {isPrivateSeed ? (
            ...
            <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5">
              ...
              <p className="mt-2 text-sm leading-6 text-amber-800">
                {catalogueIdentity?.catalogue_status === "under_review"
                  ? "This shared subject was reported and is temporarily hidden from public surfaces. Your personal Seed history is safe and remains visible to you."
                  : "You can keep growing this Seed now, but it stays private until the shared subject is approved or merged by UIN."}
              </p>
            </div>
          ) : (
            <>
                ...
                {SEED_VISIBILITY_OPTIONS.map((option) => (
                  ...
                  >
                    <input
                      ...
                      className="sr-only"
                    />
                    <span className="block text-sm font-bold text-gray-950">{option.label}</span>
