...
export default function DiscoverPersonalIntentCard({ item }: { item: DiscoverPersonalIntent }) {
  ...
  return <article className="flex h-[400px] min-w-0 flex-col overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm transition hover:shadow-md">
    <Link href={`/seeds/${encodeURIComponent(item.source_seed_id)}`} className="flex min-h-0 flex-1 flex-col">
